...
void UGravityMovementcomponent::TickComponentBase(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (
		...
		(!bCheatFlying || bIsSimulatingPhysics) && 
		!true // CharacterOwner->CheckStillInWorld()
		)
	{
		return;
	}
	...
	if (CharacterOwner->GetLocalRole() > ROLE_SimulatedProxy)
	{
		...
		else if (CharacterOwner->GetRemoteRole() == ROLE_AutonomousProxy)
		{
			...
			if (
//				CharacterMovementCVars::NetEnableListenServerSmoothing && 
...
{
	TickComponentBase(DeltaTime, TickType, ThisTickFunction);

	PreStepUpLocation = UpdatedComponent->GetComponentLocation();

	GravityDirection = (GravityOrigin - PreStepUpLocation).GetSafeNormal();

	CapsuleQuat = UpdatedComponent->GetComponentQuat();

	const auto NewRot = UKismetMathLibrary::MakeRotFromZX(-GravityDirection, CapsuleQuat.GetAxisX());
...
)
{
	...
	if (IsMovingOnGround())
	{
		...
		if (NormalZ > 0.f)
		{
			if (!IsWalkable(Hit))
			{
		//		DrawDebugDirectionalArrow(GetWorld(), GetActorLocation(), GetActorLocation() + (Normal * 100), 20, FColor::Red, false, 1);
				Normal = UKismetMathLibrary::ProjectVectorOnToPlane(Normal, GravityDirection);
		//		DrawDebugDirectionalArrow(GetWorld(), GetActorLocation(), GetActorLocation() + (Normal * 100), 20, FColor::Green, false, 1);
			}
		}
